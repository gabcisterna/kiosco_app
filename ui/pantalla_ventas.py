...
class PantallaVentas:
    def __init__(self, master):
        ...
        tk.Label(top_frame, text="Buscar (ID empleado o nombre cliente):", bg='#f0f0f0', font=("Segoe UI", 10)).pack(side=tk.LEFT, padx=5)
        self.entry_buscar = tk.Entry(top_frame, textvariable=self.busqueda_var, font=("Segoe UI", 10))
        ...
        opciones_orden = ["Fecha descendente", "Fecha ascendente", "Total descendente", "Total ascendente"]
        self.combo_orden = ttk.Combobox(top_frame, values=opciones_orden, state="readonly", textvariable=self.orden_var, width=20)
        ...
        self.combo_filtro = ttk.Combobox(top_frame, values=opciones_filtro, state="readonly", textvariable=self.filtro_var, width=20)
        ...
        columns = ("fecha", "empleado", "cliente", "total", "pago")
    ...
    def actualizar_lista(self):
        ...
        if busqueda:
            ...
            for v in ventas:
                ...
                if busqueda in id_empleado_str or busqueda in nombre_cliente:
                    ventas_filtradas.append(v)
            ventas = ventas_filtradas

        reverse = "descendente" in orden
        if "Fecha" in orden:
            ventas.sort(key=lambda v: datetime.strptime(v["fecha"], "%Y-%m-%d %H:%M:%S"), reverse=reverse)
        elif "Total" in orden:
            ventas.sort(key=lambda v: v["total"], reverse=reverse)
        ...
        for v in ventas:
            ...
            self.tree.insert("", tk.END, values=(fecha_hora, nombre_emp, nombre_cliente, f"${v['total']:.2f}", v["forma_pago"]))

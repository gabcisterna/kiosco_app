...
from modules.rutas import ruta_datos
...
class PantallaEmpleados:
    ...
    def estilizar_widgets(self):
        ...
        style.configure("Treeview.Heading", font=("Segoe UI", 10, "bold"), background="#dee2e6")
        style.configure("Agregar.TButton", font=("Segoe UI", 10, "bold"), background="#38b000", foreground="white", padding=6)
        style.map("Agregar.TButton", background=[("active", "#2a8800")])
        style.configure("Editar.TButton", font=("Segoe UI", 10), background="#1d84b5", foreground="white", padding=6)
        style.map("Editar.TButton", background=[("active", "#156a96")])
        style.configure("Eliminar.TButton", font=("Segoe UI", 10), background="#e63946", foreground="white", padding=6)
        style.map("Eliminar.TButton", background=[("active", "#c91828")])
    ...
    def mostrar_vista_turnos(self):
        ...
        tk.Label(self.content_frame, text="Historial de Turnos", bg="#e9ecef", font=("Segoe UI", 14, "bold")).pack(anchor="w")
    ...
    def cargar_turnos(self):
        ...
        for turno in reversed(turnos):
            fin = turno["fin"] if turno["fin"] else "En curso"
            self.tree_turnos.insert("", tk.END, values=(turno["empleado"], turno["inicio"], fin))
    ...
    def mostrar_vista_empleados(self):
        ...
        self.entry_id.bind("<KeyRelease>", lambda event: self.buscar_empleado_dinamico())

        ttk.Button(top_frame, text="Agregar Empleado", command=self.agregar_empleado_ui, style="Agregar.TButton").pack(side=tk.RIGHT)
        ...
        self.tree.column("Puesto", width=160)
        self.tree.pack(pady=10, fill=tk.BOTH, expand=True)

        bottom_frame = tk.Frame(self.content_frame, bg="#e9ecef")
        bottom_frame.pack(pady=10)

        ttk.Button(bottom_frame, text="Editar", command=self.editar_empleado_ui, style="Editar.TButton").pack(side=tk.LEFT, padx=10)
        ttk.Button(bottom_frame, text="Eliminar", command=self.eliminar_empleado_ui, style="Eliminar.TButton").pack(side=tk.LEFT, padx=10)
    ...
    def buscar_empleado_dinamico(self):
        ...
        for empleado in empleados:
            if id_buscado == "" or empleado["id"].lower().startswith(id_buscado):
                self.tree.insert(
                    "",
                    tk.END,
                    values=(
    ...
    def agregar_empleado_ui(self):
        ...
        puestos_disponibles = [puesto for puesto in listar_puestos_disponibles() if puesto != PUESTO_DUENO]
        combo_puesto = ttk.Combobox(ventana, values=puestos_disponibles, state="readonly")
        ...
        def confirmar():
            ...
            empleados = cargar_empleados()
            ids_existentes = [int(empleado["id"]) for empleado in empleados if empleado["id"].isdigit()]
        ...
        tk.Button(ventana, text="Agregar", command=confirmar).grid(row=3, column=0, columnspan=2, pady=10)
    ...
    def editar_empleado_ui(self):
        ...
        tk.Label(ventana, text="Puesto:").grid(row=2, column=0)
        combo_puesto = ttk.Combobox(ventana, values=listar_puestos_disponibles(), state="readonly")
        ...
        def confirmar():
            ...
            nuevos_datos = {
                ...
                "correo": nuevo_correo,
                "puesto": nuevo_puesto,
                "usuario": nuevo_correo or nuevo_nombre,
            }
        ...
        tk.Button(ventana, text="Actualizar", command=confirmar).grid(row=3, column=0, columnspan=2, pady=10)
